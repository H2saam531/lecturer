...
interface researc_list{
  quiz_comment: string
  quiz_dateOfappointed: string
  quiz_deliveryDate: string
  quiz_discussion: number
  quiz_id: number
  quiz_idcf: number
  quiz_iduf: number
  quiz_maximumNumberOfGroup: number
  quiz_minimumNumberOfGroup: number
  quiz_name: string
  status: string
}
interface GetStatus{
  ...
  const url = "https://secend-pr.shuttleapp.rs/todos";
    ...
    const onSubmit = async (values: any, actions: any) => {
      ...
      const reqponse = await fetch(`${url}/add-status-quiz`,{
        method: "POST",
        body: JSON.stringify({
          // "type_of_element": Object.keys(params)[0],
          // "status_idcf": parseInt(params.quiz),
...
const getStatus = async () => {
  toast.error("Error 400");
    if (auth != null){
      toast.error("Error 400");
        fetch(`${url}/quiz-get-status`,{
          method: "POST",
          body: JSON.stringify({
            // "courseid": parseInt(params.quiz),
  ...
  for (let j = 0; j < cLen; j++) {
    ...
    if(get_status[i].id == corse_lest[j].quiz_id){
...
if(empathy != "empathy"){
    ...
    if (isNull){
      ...
      const advancedSchema = yup.object().shape({
        couseType: yup.string().oneOf(corse_lest.map(op => (op.quiz_id.toString() )),"String").required("Required"),
...
return(
  ...
  // acceptedTos: false 
      ...
      <Form className="md md:text-lg text-xs w-full">
        ...
        <CustomSelect
          ...
          {corse_lest.map(option => (
            <option value={option.quiz_id} key={option.quiz_id}>{option.quiz_name}</option>
          ))}
    ...
    <div className="w-full  rounded-md h-14 border shadow-2xl text-center flex flex-row justify-right items-center">
      ...
      {corse_lest.filter(
        ...
        (d) => {if(check == true){ return new Date(d.quiz_deliveryDate).valueOf() - new Date().valueOf() > 0}
      else{
        return d
      }
      }
  ...
  <div className="layer relative flax flax-row rounded-md shadow-xl w-full " key={post.quiz_id}>
    ...
    {/* <div className="" key={post.research_id}>  */}
      ...
      <span className="basis-1/4"><h2>اسم البحث :         <div className="underline font-semibold decoration-solid inline">{post.quiz_name.toString()}</div></h2></span>  
    
        <span><h2>تاريخ التحديد :     {post.quiz_dateOfappointed}</h2></span>  
        <span><h2>تاريخ التسليم  :    {post.quiz_deliveryDate}</h2></span>  
        <span><h2>اقل عدد للمجموعة :  {post.quiz_minimumNumberOfGroup}</h2></span>  
        <span><h2>اكبر عدد للمجموعة : {post.quiz_maximumNumberOfGroup}</h2></span>  
        <span className="basis-1/4"><h2>تعليق : {post.quiz_comment}</h2></span>  
    ...
    value={post.quiz_id}  onClick={() => UpdateData(post.quiz_id, index)} data-id={index} disabled={isDisabled}>update</button>
    <button 
    ...
    value={post.quiz_id} onClick={() => DeleteData(post.quiz_id, index)} data-id={index} disabled={isDisabled}>deleate</button>
  </div> 
  </div>
